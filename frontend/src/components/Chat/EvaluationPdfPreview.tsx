...
const EvaluationPdfPreview: React.FC<Props> = (props: Props) => {
  ...
  const handleDownloadPdf = () => {
    ...
    doc.setFontSize(18);
    doc.text(`INFORME DE INTERACCIÓN CON ${character.toUpperCase()}`, 40, 50);
    ...
    <div className="fixed inset-0 bg-black/40 z-40 flex items-center justify-center p-4">
      <div className="w-[880px] max-w-full bg-white rounded-lg shadow-xl overflow-auto max-h-[90vh]">
        <div className="flex items-center justify-between px-6 py-4 border-b">
          <h3 className="text-lg font-semibold">Vista previa: Informe de Evaluación</h3>
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="mb-6">
            ...
            <div className="border rounded overflow-hidden">
              <table className="w-full text-left text-sm">
                <thead className="bg-blue-50">
                  <tr>
                    <th className="px-4 py-2">Puntaje total</th>
                    <th className="px-4 py-2">Rango de desempeño</th>
                  </tr>
                </thead>
                <tbody>
                  <tr className="bg-white">
                    <td className="px-4 py-2 font-medium">{conclusion.find(c => c.title === 'Puntaje total')?.text}</td>
                    <td className="px-4 py-2">{conclusion.find(c => c.title === 'Puntaje total')?.text.split(' - Desempeño: ')[1]}</td>
                  </tr>
                </tbody>
              </table>
            </div>
          ...
          <div>
            ...
            <div className="space-y-3">
              {conclusion
                .filter(c => c.title !== 'Puntaje total' && c.title !== 'Áreas de mejora')
